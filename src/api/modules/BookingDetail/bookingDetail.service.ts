...
export class BookingDetailService {
  ...
  async find(item: BookingDetail) {
    ...
    for (const element of response) {
      const product = await this.productService.findFirst({
        id: element.product_id,
      });
      const priceLine = await this.priceLineService.findFirst({
        id: element.price_id,
      });
      const unit_exchange = await this.unitChangeService.findFirst({
        id: element.unit_exchange_id,
      });
      ...
      const product_recived = !promotionDetail
        ? null
        : await this.productService.findFirst({
            id: promotionDetail.product_received_id,
          });
