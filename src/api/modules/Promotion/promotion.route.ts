...
routePromotion.get('/', async (req: Request, res: Response, next: NextFunction) => {
	const promotions = await promotiomService.getAll();
	res.json(promotions) 
})

routePromotion.post('/edit', async (req: Request, res: Response, next: NextFunction) => {
	const {id} = req.body
})
...
routePromotion.get('/update', async (req: Request, res: Response, next: NextFunction) => {
